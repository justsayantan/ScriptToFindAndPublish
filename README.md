...
Scenario: 
In a migration phase (Or in maintenance phase) if you need to change dynamic component template, it is really hard to republish that template especially when huge number components are associated with that template. 
...
Solution: 
To overcome this scenario I have written a script. Once we are ready with our changes in the templates then script helps to republish all the published items. The script will take inputs from user and find the resolved items with that dynamic component template and republish only those to that target so that we should not publish any unwanted(Which was not previously published) item to the target. 
I have discussed with our colleagues to take their expert suggestion and also fine-tuned this script couple of time.

Functionality: 
As input the script will accept below item –
1)     Template Id
...
3)	Re-Publish those items to the same target from particular publication .
Additionally Logging and validation of the user input is enabled on that script.
